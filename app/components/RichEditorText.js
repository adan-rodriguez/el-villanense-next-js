...
export default function RichEditorText() {
  useEffect(() => {
    ...
    function init() {
      let html = "";
      let template =
        '<button type="button" title="%tooltip%" data-command="%cmd%">%icon%</button>';
      commands.map((command) => {
        let temp = template;
        temp = temp.replace(/%cmd%/gi, command.cmd);
        temp = temp.replace(/%tooltip%/gi, command.tooltip);
      ...
      // htmlResult.textContent = richTextEditor.innerHTML;
    }
    ...
    document.querySelectorAll("button").forEach((button) => {
      button.addEventListener("click", (e) => {
        ...
        // if (cmd === "backColor" || cmd === "foreColor" || cmd === "hiliteColor") {
        //   colorPicker.showPicker();
        //   colorPicker.addEventListener("input", () => {
        //     val = colorPicker.value;
        //     execCommand(cmd, val);
        //   });
        //   return;
        // }
    ...
    //   // // Obtener y mostrar el html generado
    //   function getHtml(e) {
    //     htmlResult.textContent = e.target.innerHTML;
    //   }

    //   // // Escuchar el cambio en el editor
    //   richTextEditor.addEventListener("input", getHtml);

    // Observador de mutaciones
    const observer = new MutationObserver((mutationsList) => {
      for (const mutation of mutationsList) {
        if (mutation.target.childElementCount === 0) {
          richTextEditor.innerHTML = "<p><br></p>";
        }
      }
    });

    // Configura el observador para observar cambios en el <div>
    observer.observe(richTextEditor, { childList: true });
  }, []);

  return (
    <div className={styles.container}>
      <div className={styles.buttons_container} id="buttons-container"></div>
      <div
