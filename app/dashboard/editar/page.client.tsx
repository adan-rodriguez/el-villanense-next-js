...
export function EditArticleClientPage({
  ...
  async function handleSubmitEditArticle(e: React.FormEvent<HTMLFormElement>) {
    ...
    let response;
    try {
      response = await fetch(`/api/article/${id}`, {
        method: "PUT",
        headers: {
    ...
    if (!response.ok) {
      getLoadingUpdateArticle(false);
      getErrorMessage("Ocurrió un error. Inténtelo nuevamente");
      return;
    }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      getLoadingDeleteArticle(false);
      getErrorMessage("No se ha podido eliminar la noticia");
      return;
    }
  ...
  return (
    <>
      ...
      >
            ...
            <Button
              ...
              onClick={async () => {
                ...
                const filteredAuthors = _authors.filter(
                  (_author) =>
                    authors.findIndex((author) => author.id === _author.id) ===
                    -1
                );
        ...
        <fieldset>
          <legend style={{ fontSize: "smaller" }}>Imagen</legend>
          {changeImage ? (
            <>
